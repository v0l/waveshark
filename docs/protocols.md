...
| OFDM (FFT, pilots, equaliser), `dsp::wifi` | 802.11a/g subcarrier symbols and the channel response | yes | no |
...
| Prologue, FreeTec NC-7104, Pearl NC-7159-675, ThermoPro TX2, TFA 30.3240.10 | 433.92 MHz | OOK PPM 2/4 ms | 31 kHz | done | table | No checksum either, and only a four bit type nibble to go on, so a frame is claimed only where two of the burst's rows are 36 bits long and identical, or where a package is one such row and nothing else. rtl_433 asks for four of the seven copies the sensor sends and can, since its reset limit holds a whole transmission in one buffer; this receiver's detector cuts a package at the 9 ms gap between copies, and asking for four here read none of these sensors through the live path while the corpus harness read all of them. The Alecto V1 family (Auriol, Unitec W186-F) sends the same 36 bits at the same timings with a real checksum in the last nibble, so a frame satisfying that checksum is handed over rather than claimed. Verified against four of rtl_433's recordings, covering both type nibbles, a negative temperature, the button flag and a sensor with no humidity element |
| Ambient Weather F007TH, F012TH, SwitchDoc F016TH, TFA 30.3208.02 | 433.92/868 MHz | OOK Manchester 500 us | 31 kHz | done | table | LFSR digest, gen 0x98, key 0x3e, exclusive-ored with 0x64, behind a preamble the frame has to be found by since nothing marks its end. Both Manchester polarities are searched. Verified against three of rtl_433's recordings, one of which holds five sensors on five channels two and a half seconds apart |
...
| Ambient Weather WH31E, other Oregon Scientific, other Acurite (Atlas, 6045M lightning, 899 rain, 515 fridge) | 433.92/915 MHz | OOK PWM/Manchester | 31 kHz | table | table | Several families each, all timing tables, and the Acurite ones share the frame the tower and the 5n1 already use |
...
| Interlogix / GE / UTC security sensors, ELK-319DWM, Alula RE101 | 319.5 MHz | OOK PPM 122/244 us | 31 kHz | done | table | US patent 5761206: a 20 bit serial, a device type, a trigger count that stops a recorded packet being replayed, a low battery bit and a latch and a level for each of five inputs, so a row says which door opened, which case was lifted and which detector fired. Two parity bits are the whole integrity check and one window in four satisfies them, so the report says it has none and the frame's shape carries the weight instead: a row of 57 to 64 bits, the preamble where it belongs, and neither the serial nor the state all zeros or all ones. Verified against two of rtl_433's recordings of a motion detector, before and after its case was opened; the contact sensor its corpus holds is checked as a frame rather than as a capture, since both transmissions in that recording land while the noise floor is still settling and the auto node reads a fragment of the first and none of the second. A keyfob sends its buttons in the trigger count rather than in the latches, which is the manufacturer misreading its own patent, and is read that way here too. Device type 6 is `heat`, which older rtl_433 output calls `smoke` |
...
| 802.11a/g Wi-Fi | 2.4 GHz channels 1-14, 5 GHz U-NII | OFDM, 52 subcarriers, BPSK to 64-QAM | 20 MHz | off air | chain | `dsp::wifi` is the whole legacy OFDM receiver: the short training field found by its own repeat, the carrier offset off that and then off the two long symbols, a channel estimate per subcarrier, the SIGNAL field, and the data symbols through the deinterleaver and a K=7 Viterbi to a CRC-32 that decides. `decode::wifi` reads the MAC frame: type, addresses, whether the address is randomised, and for a beacon or probe the SSID, the channel it claims and its security. All eight legacy rates, 6 to 54 Mbit/s, which is what management traffic is sent at. Verified off air against `ofdm_wifi_frames_2462M_20000k.cs8`, where the receiver reads 81 frames and every one of them passes its FCS, naming the access point and the station by the addresses they transmitted. Two things a reader should know. The channel is 20 MHz, so this is a HackRF or a LimeSDR and never an RTL-SDR. And a span wider than one channel is read a channel at a time: `WifiSpan` puts a mixer and a receiver on each channel the span covers, and it starts on the channels that cover the band rather than on all of them: at 2.4 GHz the channels overlap on a 5 MHz grid, so reading thirteen is reading each transmission three times for three times the work. It starts on 1, 6, 11, 13 and 14, and opens another when a beacon's DS parameter set says its network is there, which is heard on the neighbours even when the network cannot be read there. They run in parallel, and a channel whose band is within three decibels of its own floor is skipped whole, extraction included, which is where most of the saving is: measured on a 61.44 MS/s capture, eight channels went from five times slower than real time to a little faster than it, and an empty band from six times faster to a hundred. A channel that has been asleep keeps what it learned about its noise and drops only its buffers, and is handed the previous block on waking, because the transmission that woke it began in that one, since a channel here is a third of the span rather than a fiftieth and no shared bank helps. That rate reaches 20 MS/s through `dsp::resample`, because 61.44 divided by three is 20.48 and 802.11 will not tolerate being two percent out. And the generator polynomials are written into the shift register reversed: 133 and 171 octal are defined with the newest bit at the top and the register here holds it at the bottom, so an encoder given the numbers straight decodes its own output perfectly and reads nothing off the air. 802.11b is read separately, below |
| 802.11b Wi-Fi (HR/DSSS) | 2.4 GHz channels 1-14 | DBPSK and DQPSK spread by the 11-chip Barker sequence at 11 Mchip/s | 22 MHz | off air | chain | 1 and 2 Mbit/s, which matters more than the rate suggests: every access point sends its beacon at 1 Mbit/s, so a receiver that reads only OFDM hears the traffic and never the announcements. `dsp::wifi::dsss` gates on the envelope, despreads with an interpolated Barker correlator (a symbol is one microsecond, which is exactly twenty samples at this receiver's rate, so there is no timing loop: only the phase within those twenty has to be found), reads the differential phase, descrambles with the self-synchronising register and checks the PLCP header's own CRC-16 before the frame's FCS. Verified off air against `ofdm_wifi_2462M_20000k.cs8`, whose 3.4 ms bursts every 102.4 ms that manifest recorded as unidentified are beacons: they decode to a network name, an access point and a security setting. Not read: 5.5 and 11 Mbit/s, which replace the Barker sequence with complementary code keying and need a different receiver, and which carry data the OFDM side usually has anyway. Two things a reader should know. The signal is 22 MHz wide and the channel this cuts is 20, so about a tenth of its energy is outside the stream, which costs correlation and no packets. And because it is wider than a channel, one transmission is decoded by several overlapping channel receivers at once, so `WifiSpan` reports identical bytes at the same instant once, at whichever channel heard it best |
| 802.11n Wi-Fi (HT mixed) | as above | OFDM, 56 subcarriers, MCS 0-7 | 20 MHz | off air | chain | One spatial stream, which is what one aerial can receive however many were sent, so MCS 0 to 7 and not the two-stream rates above them. `dsp::wifi` reads the HT header out of the two symbols after the legacy one, keyed a quarter turn round so it cannot be mistaken for a legacy SIGNAL field and guarded by its own eight bit check, measures the channel again on the HT training symbol (four subcarriers wider than the legacy one), and reads the data at either guard interval. An aggregate is taken apart into the MAC frames inside it, each with its own FCS and each a row of its own. Verified off air on the same capture: three MCS 7 aggregates at the short guard interval, decoding to QoS data frames between the access point and a station that the legacy frames around them name. Not read: 40 MHz, greenfield, LDPC, space-time coding, and anything above MCS 7. The pilots turn one subcarrier every symbol in an HT frame where a legacy frame's stand still, which is worth knowing because a receiver that misses it decodes the first symbol of every frame and none of the rest |
| Wi-Fi channel state information | as above | as above | 20 MHz | done | no | The channel response the equaliser already measures, carried on the frame it belongs to and on the transmitter's address. This is what WiFi sensing is built on. What it is not is a sensing system: those want a stream of estimates at a steady rate from a link that keeps transmitting, and a passive receiver gets whatever the access point happened to send |
...
| Open Drone ID over Wi-Fi Beacon and NAN | 2.4 and 5.8 GHz, 20 MHz channels | 802.11 OFDM | 20 MHz | synthetic | chain | Both transports, on the 802.11 receiver that already reads the band. A beacon carries a vendor specific element (id 221) under the ASD-STAN OUI FA:0B:BC with type 0x0D, a message counter, and then either one message or a pack; a NAN broadcast is a public action frame (category 0x04, code 0x09) under the Wi-Fi Alliance OUI 50:6F:9A type 0x13, whose service descriptor attribute is recognised by the service id 88:69:19:9D:92:09, the hash of `org.opendroneid.remoteid`, with the pack behind the service info header. The attributes are walked rather than assumed to be in the reference sender's order, since only the service id says a descriptor is Remote ID and not another NAN service. `decode::wifi` hands over the vendor elements and the action payload without reading either, the same division `decode::ble` keeps, and `nodes::wifi_nodes` names the row for the aircraft when a message is present. The payload parser is the one the Bluetooth rows use. Checked against the vendor element published in `opendroneid/transmitter-linux`'s `beacon.conf` and against frames built to the reference library's layout; what is missing is a capture of an aircraft doing it. Note 6A:5C:35 with type 0x01 is a different thing that this deliberately does not read: the French national scheme, a TLV encoded message set of its own |
| DJI DroneID | 2.4 and 5.8 GHz, on fixed centres 15 MHz apart | OFDM, LTE numerology, 600 carriers 15 kHz apart, QPSK | 15.36 MS/s | off air | no | Broadcast roughly twice a second by DJI aircraft independently of Remote ID, in the clear, which DJI described as encrypted until the NDSS 2023 paper showed otherwise. It carries what Remote ID does and more: the airframe serial as printed on it, position, height, velocity, the home point, the operator's own position and a free text identifier. A burst is nine OFDM symbols, the fourth and sixth Zadoff-Chu sequences with roots 600 and 147 rather than anything the LTE specification would suggest, and no pilots at all, so the timing, the frequency and the channel all come from those two symbols. `dsp::droneid` finds a burst by correlating the first of them, takes the carrier offset off the second symbol's cyclic prefix, equalises on the ZC channel estimate, reads the QPSK and descrambles with LTE's own gold sequence; `decode::droneid` reads the 91 byte frame under its CRC-16. Verified off air against a DJI Mini 4K in `testdata/fixtures.toml`: seven frames, one serial, consecutive sequence numbers, and every position field absent because the aircraft was indoors without a fix. Three things a reader should know. The block is read systematically rather than turbo decoded: LTE's rate matcher is there (the transmitted sequence repeats every 4236 bits, which is LTE's 4320 minus its 84 punctured positions, and the block starts at k0 = 90), and the systematic stream alone passes both the CRC-24 over the block and the CRC-16 over the frame, but the parity streams do not fit the LTE turbo encoder as `dsp::lte_turbo` builds it, so there is detection and no correction and a weak burst is simply lost. The first OFDM symbol carries no data and descrambles to all zeros, which is the one end-to-end check available before any CRC and is what proved the physical layer while the coding was still wrong. And DJI sends a second packet type interleaved with this one, six of the thirteen bursts in that capture, whose layout differs and which is not read yet |
...
| ExpressLRS 2.4 GHz | 2400.4-2479.4 MHz, 80 channels 1 MHz apart | LoRa SF5-SF8 over 812.5 kHz, or FLRC at 1 Mbps | 2 MHz | framing, demod for FLRC | mod | An SX1280. `decode::elrs` reads the link layer: the CRC-14 seeded from the binding UID, the four packet types, a sync packet's hop index, counter, rate and the two UID bytes it sends in the clear, the four ten bit channels an RC packet carries, and the hop sequence a UID generates, all from the firmware's own `src/lib/OTA` and `src/lib/FHSS`. Confirmed off air against a TX16S: SF7 over 812.5 kHz, sync word 0x12, on channel after channel of the hop set. Which of the ten rates is running is measured rather than configured, the way every other front end here decides things: the sweep the classifier reads off a burst gives the spreading factor (5.16e9 Hz/s over 812.5 kHz is SF7 and nothing else), and where that leaves two rates, the spacing between packets that stayed on one channel separates them, since the link keys on a fixed interval and hops every four packets. On the bench capture that measures 10.00 ms, which is 100 Hz Full, and the handset was set to 100 Hz Full. The SX1280 transmits with I and Q swapped against the SX127x convention, so the preamble is downchirps to this receiver and the samples have to be conjugated before `dsp::lora` sees anything; five captures read as an empty band before that was found. `decode::lora_li` reads the payload: the SX1280 long interleaved coding was measured off a bench SX1281 and confirmed against this handset, and the packets it produces carry sticks in microseconds under a CRC that agrees across the capture on one seed. `nodes::ElrsNode` is the front end, placed by the auto node on a 2.4 GHz source the classifier has named a chirp and offered as a strip mode like any protocol; it shares the LoRa node's reader and reads the symbols through `decode::lora_li`. Once a packet of a link has decoded it publishes a lock over the eighty channels (`pipeline::lock`), and from then on the auto node hands it every visit the detector opens on one of them without classifying it first, since the hop set is a megahertz raster whatever the UID is. The link it needs for the CRC is learned rather than configured: from a sync packet where the span holds the sync channel, and otherwise from the packets themselves, since consecutive packets on one channel have CRC seeds that share a high byte and either agree or count, and `decode::elrs::recover_link` reads the seed back through the CRC's linearity rather than searching for it. The bench capture (`elrs_100hz_2415M_20000k.cs8`, a TX16S at 100 Hz Full) has no sync packet in its 20 MHz and reads 224 of 240 packets that way, link 6f37, sticks centred and throttle low. Getting there needed three things that no synthetic packet had shown: the detector refused any source wider than 600 kHz, the classifier measured the four packets of a channel visit as one keyed burst and scored a chirp with a keyed envelope as nothing, and the 812.5 kHz channel was not one LoRa knew. Hop following, meaning knowing which channel is next rather than which eighty it will be one of, is still missing, because two bytes of a UID cannot generate the sequence; a binding phrase given to the node can. The coding is measured only at SF7 and 4/8, so the other rates decode as nothing rather than as guesses. [Getting the SX1280's coding out of an SX1280](#getting-the-sx1280s-coding-out-of-an-sx1280) is the procedure for closing that. FLRC is a coherent GFSK burst mode with its own coding and is a front end of its own |
...
so. Either way the same parser reads the drone and the Holybro module, over
Bluetooth advertising or in a beacon or NAN frame on the 802.11 receiver, so
what a capture of the aircraft buys is a row moving from **synthetic** to
**off air** rather than a front end. ELRS gives a hopping
...
| Protocol | Where | Modulation | Width | RX | TX | Notes |
|---|---|---|---|---|---|---|
| GSM synchronisation channel | 850/900/1800/1900 MHz | GMSK 270.833 kbps | 200 kHz | synthetic | mod | `dsp::gsm` finds the frequency correction burst by the variance of its phase advance, which is a tone a quarter of the symbol rate up, and reads the synchronisation burst one TDMA frame later: the cell's identity code and the frame number, behind ten bits of parity that decide whether a burst happened at all. The tone also measures the tuner's error, which the burst after it needs to a few hundred hertz. The tone is found by the coherence of the phase advance across a burst rather than by its variance, which is what makes it measurable at the signal to noise ratio a real cell arrives at, and a burst is reported only when a second one agrees with it about the frame number: ten bits of parity let one burst in a thousand through, and a scan tries thousands. The scanner table scans the four downlink allocations with `auto`, which finds the carriers and places this front end on every source that measures 200 kHz: which channels a network uses is licensed per operator and per country, so a carrier is detected rather than listed. The `[GSM]` block beside them pins one carrier and watches only it, and ships switched off. Checked against a live network off air, which is where both of its bit orders came from, but no capture of one is committed: a cell identity plus a channel number places a receiver within a few hundred metres, and that would be in the history for good. What is committed instead is `a_multiframe_of_beacons_keeps_time_with_itself`, which is that recording's shape and impairments with the identity replaced by the test network 001-01, and two vectors that pin the bit orders from outside this crate. `crates/decode/examples/gsm_scan.rs` says whether a capture holds a cell and what it decodes; keep the capture local |
| GSM broadcast and common control | as above | as above | 200 kHz | synthetic | mod | The four frames after each synchronisation burst, read as normal bursts and deinterleaved into one 23 byte block behind a 40 bit Fire code. Nothing marks a burst as broadcast: the receiver knows where the block is because the synchronisation burst said which frame it was in, and knows which of the eight training sequences to correlate because the standard makes it the cell's own colour code. `decode::gsm` then reads the cell identity and the location area out of system information 3, 4 and 6, the cell's own hopping allocation out of type 1 and the neighbours a phone is told to measure out of type 2, and names the rest. Paging requests are read too, all three types: a network calls a phone by a temporary identity it reallocates, so a run of them says how busy a cell is without saying whose phones they are, and the row separates out the ones paged by permanent subscriber identity because a network doing that has given up the point of the temporary one. A page is a link, this cell calling one handset, so a request naming four phones becomes four rows and four links, the way a pager transmission becomes a row per address. What the phone is called decides what kind of party it is: an IMSI or an IMEI is a unit, a TMSI is `PartyKind::Temporary`, a name the network hands back. None of them is a device. Only the downlink is read here, so a paged phone has transmitted nothing this receiver heard; recording it in the survey would put a position on a handset that may be switched off three cells away, and a reallocated identity would fill the database with sightings of nobody. On a live cell about 2% of pages were by permanent identity, some of them roamers. An immediate assignment says which channel a phone was just granted, hopping or not, and carries the timing advance the network told it to use, which is how long its burst took to arrive and therefore the range to it: 554 metres a step, and the furthest phone in a recording was 2.8 km away. A cell that names itself fully becomes a device in the survey, keyed by operator, location area and cell number rather than by the carrier it was heard on, so moving a cell to another channel does not make it a new one. The decode says who it is, as `common::Identity`, the way every other protocol does; a synchronisation burst carries only a colour code, which is reused a few streets away, so it claims no identity at all. The neighbour list is what makes one cell worth reading: it is the channel numbers of the others, so a receiver that has read one carrier has been handed the rest of the network. All five frequency list formats are read. Bit map 0 is one bit per channel and covers the 900 band; the 1800 band's numbers run past a thousand, so its lists are sent as a tree of differences instead, and that recursion is transcribed from Wireshark's `f_k` rather than reproduced from the prose, because nobody gets it right from the prose and a list decoded wrongly reads as neighbours that are not there. What a burst was heard at is measured where the burst sat rather than over the block it arrived in, and its signal to noise ratio comes from the equaliser's own fit, `q / (1 - q)` of the power the channel estimate explains: a beacon transmits continuously, so there is no quiet part of the channel for a noise floor to be measured against, and the shared frame meter read nought for every burst. Every burst is read through the channel the training sequence measures, in `dsp::gsm::equalise`: five taps estimated by least squares and a max-log-MAP pass over the trellis they define. Slicing the symbols instead works on a synthesised signal and falls apart on a real one, because GMSK spreads every symbol over three symbol periods before any reflection does |
...
12. ~~**Open Drone ID over Bluetooth, both transports.**~~ Done, off air
   ...
   ships with.
13. ~~**DJI DroneID.**~~ Done, off air against a Mini 4K: the OFDM front end,
   the ZC detector and the frame, read systematically rather than turbo
   decoded. What is left is the parity streams, which would buy the weak
   bursts, and the second packet type. It was the most expensive thing in
   this file and the highest value, because it is transmitted in the clear
   and names the airframe.
