...
| Protocol | Where | Modulation | Width | RX | TX | Notes |
|---|---|---|---|---|---|---|
| Open Drone ID over Bluetooth legacy | 2402/2426/2480 MHz | GFSK 1 Mbps, BLE advertising | 2 MHz | synthetic | mod | ASTM F3411 and EN 4709-002, the same message set in both. A legacy advertisement carries service UUID 0xFFFA, AD type 0x16, application code 0x0D and one 25 byte message: basic id (serial or session id and UA type), location (position, altitude, speed, track, timestamp), self id, system (the operator's position and the area a swarm covers) and operator id. `dsp::ble` reads the advertisement and `decode::ble` hands over the AD structures, so this cost a payload parser and nothing else: `decode::odid` reads a single message and a message pack, and `nodes::ble_nodes` names the row for the aircraft rather than for a Bluetooth device when one is present. Checked against messages built from the reference library's layout, not yet against a transmitter. Unauthenticated by design: anything received is what the transmitter chose to say, and a field the specification marks absent (a position of exactly 0, an altitude of -1000 m) is reported as absent rather than as a position in the Atlantic |
...
12. ~~**Open Drone ID over Bluetooth legacy.**~~ Parser written, on frames
   built from the published layout. What it needs now is a capture of the
   Holybro module with BT4 legacy alone enabled and a serial and position we
   set beforehand, which takes it to **off air**.
