...
worse than none, because it is believed. It is also where a stage is set by
hand. Every node describes its own knobs as data, which is what lets a chain be
saved and reloaded without each stage writing serialisation glue; the same
description renders the controls, so a decoder the interface has never heard of
gets a working panel.
