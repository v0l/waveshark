...
/// What a transmission's codewords say: one layer per page.
///
/// A transmission carries a transmitter's whole queue, so it is several pages
/// to several pagers, and each is a statement of its own. The pager is the
/// party addressed and never the subject: a page is transmitted by the
/// network, and a directory that read the address as the sender had every
/// pager in the county transmitting.
    ...
    parse(&codewords)
        ...
        .map(|m| {
            let (kind, text) = match &m.body {
                Body::Tone => ("tone", None),
                Body::Numeric(s) => ("numeric", Some(s.clone())),
                Body::Alpha(s) => ("alpha", Some(s.clone())),
            };
            // A page is written to somebody whether a person typed it or an
            // alarm system did: either way it is addressed to whoever carries
            // the pager.
            Proto::new("pocsag", kind)
                .between(Link { from: None, to: Some(Party::unit(m.address.to_string())) })
                .maybe(text.map(Fact::message))
        })
        .collect()
}
...
mod tests {
    ...
    fn the_unreversed_reading_of_that_page_is_not_the_right_one() {
        ...
        let straight: String =
            (0..5).map(|i| NUMERIC[(data >> (16 - 4 * i) & 0xF) as usize]).collect();
