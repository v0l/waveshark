...
fn temperature_c(msg: &[u8]) -> Result<f64, DecodeError> {
    ...
    let mut t =
        ((msg[5] >> 4) as f64 * 100.0 + (msg[4] & 0x0f) as f64 * 10.0 + (msg[4] >> 4) as f64)
            / 10.0
...
fn keep_worse(best: Result<Report, DecodeError>, e: DecodeError) -> Result<Report, DecodeError> {
...
mod tests {
    ...
    fn payload(
        ...
        msg[5] =
            (((t / 100 % 10) as u8) << 4) | if temp_c < 0.0 { 0x08 } else { 0 } | (t / 1000) as u8;
