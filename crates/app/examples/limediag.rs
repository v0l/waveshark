...
fn main() {
    ...
    println!(
        "{chan} / {ant} at 95.8 MHz, 4 MS/s, chip {:.0} C\n",
    ...
    println!(
        "test tone   floor {f0:>7.1}  peak {p0:>7.1} dBFS at {:+.0} kHz  ({:.1} dB up)",
        off / 1e3,
        p0 - f0
    );
    ...
    for g in [0.0f32, 20.0, 30.0, 40.0, 50.0, 60.0, 73.0] {
        ...
        println!(
            "  {g:>4.0} dB   floor {fl:>7.1}  peak {pk:>7.1} at {:+7.0} kHz  ({:>4.1} dB up)",
            off / 1e3,
            pk - fl
        );
    }
    ...
    for a in ["LNAH", "LNAL", "LNAW"] {
        ...
        let (fl, pk, off) = floor_and_peak(&mut d);
        println!(
            "  {a:<5}    floor {fl:>7.1}  peak {pk:>7.1} at {:+7.0} kHz  ({:>4.1} dB up)",
            off / 1e3,
            pk - fl
        );
    }
