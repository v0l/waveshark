...
mod tests {
    ...
    fn a_tpv_object_is_a_fix() {
        let line = r#"{"class":"TPV","device":"/dev/ttyACM0","mode":3,"time":"2026-09-07T09:42:50.000Z","lat":53.513700,"lon":-6.243100,"altMSL":42.5,"speed":13.2,"track":271.4}"#;
        let f = parse_tpv(line).expect("a fix");
        assert!((f.lat - 53.5137).abs() < 1e-6);
        assert!((f.lon + 6.2431).abs() < 1e-6);
