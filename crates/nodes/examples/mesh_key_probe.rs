...
fn main() {
    ...
    for line in text.lines() {
        let bytes: Vec<u8> = (0..line.len() / 2)
            .filter_map(|i| u8::from_str_radix(&line[i * 2..i * 2 + 2], 16).ok())
            .collect();
        let Some(r) = decode::lora::Received::parse(&bytes) else {
            continue;
        };
        ...
        if force || Some(m.channel_hash) == chan.hash() {
            ...
            if let Some(d) =
                decode::meshtastic::Decoded::under(ct, m.source, m.packet_id, chan.key().unwrap())
            {
                opened += 1;
                eprintln!("  {:08x} -> {:?}", m.source, d.message);
            } else {
                let plain = decode::meshtastic::decrypt_with(
                    ct,
                    m.source,
                    m.packet_id,
                    chan.key().unwrap(),
                ...
                eprintln!(
                    "  {:08x} id {:08x} hash {:02x}: {} bytes, did not open: {}",
                    m.source,
                    m.packet_id,
                    m.channel_hash,
                    ct.len(),
                    decode::channel_keys::hex(&plain)
                );
            }
        }
    }
