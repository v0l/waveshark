...
impl Simple for WigleNode {
    ...
    fn process(&mut self, i: &Payload, _o: &mut Payload, _c: &mut NodeCtx<'_>) -> Result<()> {
        ...
        for p in i.as_packets().unwrap_or(&[]) {
            ...
            // Everything else a survey records, an aircraft, a pager, a tyre
            // sensor, has no type in this format and is not filed under one
            // that happens to fit.
            ...
            let row = survey::wigle::row(
                &protocol,
                &ident,
                crate::survey_nodes::name_of(p).as_deref(),
                crate::survey_nodes::vendor_of(p).as_deref(),
                &s,
            );
            if let Some(row) = row {
...
mod tests {
    ...
    fn node_with_account(dir: PathBuf) -> WigleNode {
        ...
        n.set_station(Some(gps::Fix { lat: 53.5137, lon: -6.2431, ..Default::default() }));
        n
    }
