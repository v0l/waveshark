...
const IDENTITY: &[(&str, &str, &str)] = &[
    ...
    ("Mode S", "icao", "adsb"),
    ("AIS", "mmsi", "ais"),
    // A cell, by the identity it is known by everywhere: operator, location
    // area and cell number. Only the messages that carry the whole of it
    // become devices; a synchronisation burst names a colour code, which is
    // reused a few streets away and is not an identity.
    ("GSM-SI", "cell", "gsm"),
    ("APRS", "from", "aprs"),
    ("AX25", "from", "ax25"),
    ("M17", "src", "m17"),
