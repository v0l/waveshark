...
- Transmit a page, an APRS beacon or an RTTY over, on a channel already
  decoding one.
...
- A whole 2.4 GHz band at 61.44 MS/s keeps up better with Wi-Fi, Bluetooth
  and DroneID all reading it at once. Each front end skips a channel nothing
  is transmitting on, the drone correlator runs once per centre rather than
  once per signal the detector finds, and the history the receiver keeps for
  reopening a source no longer stalls a block to shuffle 148 MB. The worst
  block on that capture went from a twenty-fifth of real time to a
  fourteenth, and the usual block from 0.45 to 0.51.
